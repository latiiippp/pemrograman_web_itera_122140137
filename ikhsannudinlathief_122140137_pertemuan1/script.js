...
let statusKelulusan = nilai >= 60 ? "Lulus" : "Tidak Lulus";
...
document.getElementById("result").innerHTML += `
<p>Hari ini adalah hari <strong>${namaHari}</strong></p>
`;
...
for (let i = 0; i < nilaiSiswa.length; i++) {
  ...
  document.getElementById("daftar-nilai").innerHTML += `
    <li>Nilai ke-${i + 1}: ${nilaiSiswa[i]}</li>
    `;
}
let rataRata = total / nilaiSiswa.length;
document.getElementById("rata-rata").innerHTML = `Rata-rata: ${rataRata}`;
...
document.getElementById("array-demo").innerHTML += `
  <p><strong>Setelah pop:</strong> ${buah.join(
    ", "
  )} (item dihapus: ${itemDihapus})</p>
`;
...
const daftarBuah = buah.map(
  (item, index) => `${item} (Rp${hargaBuah[index].toLocaleString()})`
);
...
document.getElementById("objek-demo").innerHTML += `
  <p><strong>Hobi setelah dihapus:</strong> ${
    mahasiswa.hobi ? mahasiswa.hobi.join(", ") : "Tidak ada data hobi"
  }</p>
`;
// Manipulasi DOM
